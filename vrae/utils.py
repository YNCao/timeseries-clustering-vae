...
def open_data(direc, ratio_train=0.8, dataset="ECG5000", delimiter=None):
    """Input:
    direc: location of the UCR archive
    ratio_train: ratio to split training and testset
    dataset: name of the dataset in the UCR archive"""
    datadir = direc + '/' + dataset + '/' + dataset
    data_train = np.loadtxt(datadir + '_TRAIN', delimiter=delimiter)
    data_test_val = np.loadtxt(datadir + '_TEST', delimiter=delimiter)[:-1]
